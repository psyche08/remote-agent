...
private final class ShieldWindow: NSWindow {
    ...
    // Keyboard focus stays on the target application. Physical keyboard and
    // pointer input is isolated by InputGuard at the session event tap instead
    // of by this window, so marked agent events can still reach that target.
...
final class DisplayShield {
    ...
    init(inputGuard: InputGuard = InputGuard()) {
    ...
    func confirmCoverage(timeout: TimeInterval) -> Bool {
        ...
        while Date() < deadline {
            guard inputGuard.isActive else { return false }
        ...
        return inputGuard.isActive
            && onMain { Self.windowServerShowsCoverage(pid: getpid(), screens: NSScreen.screens) }
    }
    ...
    private func create() -> (engaged: Bool, displays: Int) {
        onMain {
            if !self.windows.isEmpty && !self.invalidated && self.inputGuard.isActive {
                return (true, NSScreen.screens.count)
            }
            self.teardown()
            // The shield window deliberately ignores mouse events so the
            // agent's marked pointer actions reach the target below it. That is
            // safe only while the modifying event tap is live: without it,
            // physical input would take the same route. Treat tap creation as a
            // required part of engaging the shield and fail closed if TCC or the
            // window server refuses it.
            guard self.inputGuard.start() else { return (false, 0) }
            ...
            for screen in screens {
                ...
                // Keep the shield in ordinary screen captures, so background
                // recording and conferencing apps see black while Locked Use
                // is open. The helper's own ScreenCaptureKit filter explicitly
                // excludes this application/windows to give only the bound
                // model turn the underlying UI frame.
                window.sharingType = Self.captureSharingType
                assert(
                    window.sharingType == Self.captureSharingType,
                    "the shield must remain visible to ordinary capture clients")
            ...
            guard self.inputGuard.isActive else {
                self.teardown()
                return (false, 0)
            }
    ...
    /// `.readOnly` permits capture while preventing capture clients from
    /// mutating the window. `.none` would omit the black cover from every
    /// recorder, exposing the temporarily unlocked UI beneath it.
    static let captureSharingType: NSWindow.SharingType = .readOnly
    ...
    /// Reads the event-tap latch directly. A suppressed physical event never
    /// reaches the target application or the ordinary idle counter, so this is
    /// the authoritative signal that a person has returned during Locked Use.
    func physicalInputObserved() -> Bool {
        inputGuard.hasObservedLocalInput
    }
