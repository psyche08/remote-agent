...
public struct ComputerUseConfig: Codable, Sendable {
    ...
    public func normalized() -> ComputerUseConfig {
        ...
        // The shield and its event tap are one safeguard: allowing Locked Use
        // to opt out of the black overlay also stopped the authoritative
        // physical-input latch. An old explicit false is accepted for config
        // compatibility but narrowed to true whenever Locked Use is active.
        if lu.enabled { lu.requireDisplayShield = true }
...
public struct LockedUseConfig: Codable, Sendable {
    ...
    /// Retained for wire compatibility. Locked Use always normalizes this to
    /// true because the shield also owns the physical-input event tap.
    ...
    public init(
        enabled: Bool = false, grantDirectory: String = "",
...
/// AGENTHALO_LOCKED_USE_DIR in mac/authorization-plugin/AgentHaloLockedUse.m.
///
...
/// unlock.
public let defaultGrantDirectory = "/Library/Application Support/AgentHalo/locked-use"
