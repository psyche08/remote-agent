...
public final class SocketServer {
    ...
    private let queue = DispatchQueue(label: "remote-agent-desktop.socket")
    ...
    private func acceptLoop() {
        while true {
            ...
            guard Self.peerIsThisUser(client) else {
                close(client)
                continue
            }
            ...
            DispatchQueue(label: "remote-agent-desktop.conn").async { [router] in
                Self.serve(client: client, router: router)
            }
        }
    }

    /// Only this user may drive the desktop.
    ///
    ...
    /// connection itself is what survives a mis-set permission. Verifying the
    /// peer's *code signature* is the stronger control and belongs here once
    /// the agent binary is signed — recorded in the docs as required hardening.
    private static func peerIsThisUser(_ fd: Int32) -> Bool {
